...
// set 0 if you don't need any log
// set 1 if you don't need debug log
#if 1
#ifndef LOG_NDEBUG
// w/o LOGV/LOGD/MARK
#define	LOG_NDEBUG
...
//Add by Hsj for set preview rotate
int UVCPreview::setPreviewRotate(int rotate) {
    ENTER()
    int result = 0;
    if (isRunning()) {
        result = -1;
        LOGW("setPreviewOrientation failed:%d", rotate);
    } else if (rotate == ROTATE_90 || rotate == ROTATE_270
               || rotate == ROTATE_0 || rotate == ROTATE_180) {
...
//Add by Hsj for set preview flip
int UVCPreview::setPreviewFlip(int flip) {
    ENTER()
    int result = 0;
    if (isRunning()) {
        result = -1;
    } else if (flip == FLIP_H || flip == FLIP_V || flip == FLIP_DEFAULT) {
        previewFlip = flip;
    } else {
        result = -2;
    }
...
int UVCPreview::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
    ENTER()
    LOGD("frame_callback_obj->%d", frame_callback_obj != NULL);
...
int UVCPreview::startPreview() {
    ...
    if (!isRunning()) {
        ...
        pthread_mutex_lock(&preview_mutex);
        {
            //Add by Hsj for open camera when no mPreviewWindow
            //if (LIKELY(mPreviewWindow)) {
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
    ...
    if (LIKELY(!result)) {
        clearPreviewFrame();
        int ret = pthread_create(&capture_thread, NULL, capture_thread_func, (void *) this);
        LOGD("do_preview thread_create=%d", ret);
        ...
        if (frameMode) {
            ...
            for (; LIKELY(isRunning());) {
                ...
                if (LIKELY(frame_mjpeg)) {
                    ...
                    LOGD("uvc_mjpeg2yuyv->%ld", getTime() - start_time);
                    recycle_frame(frame_mjpeg);
                    if (LIKELY(!result)) {
                        //Add by Hsj for open camera when no PreviewWindow
...
uvc_frame_t *UVCPreview::draw_preview_one(uvc_frame_t *frame, ANativeWindow **window,
                                          convFunc_t2 convert_func, int pixelBytes) {
    // ENTER()
    int b = 0;
    pthread_mutex_lock(&preview_mutex);
    {
    ...
    if (LIKELY(b)) {
        ...
        if (convert_func) {
            ...
            if LIKELY(converted) {
                long start_time = getTime();
                b = convert_func(frame, converted, previewRotate, previewFlip);
                LOGD("convert_func->%ld", getTime() - start_time);
                if (!b) {
                    pthread_mutex_lock(&preview_mutex);
                    long start_time = getTime();
                    copyToSurface(converted, window);
                    LOGD("copyToSurface->%ld", getTime() - start_time);
                    pthread_mutex_unlock(&preview_mutex);
                } else {
...
void UVCPreview::do_capture_callback(JNIEnv *env, uvc_frame_t *frame) {
    ...
    if (LIKELY(frame)) {
        ...
        if (mFrameCallbackObj) {
            ...
            //Fix for NullPointerException with first of stopPreview()
            //and then setFrameCallback(null) add by Hsj
            //if (isCapturing()) {
